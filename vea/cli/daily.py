"""Generate a daily briefing summarizing recent activity and upcoming events."""
...
def generate(
    ...
    full_context: bool = typer.Option(False, help="Skip filtering and use all context"),
    ...
    """Generate a daily summary by filtering context and calling the LLM."""
    ...
    try:
        ...
        # include known project/person aliases as extra focus terms
        ...
        def _filter_group(name: str, docs: List[dict]) -> List[dict]:
            """Apply relevance filtering to ``docs`` and log the results."""
            if not docs:
        ...
        if full_context:
            # send every loaded document to the LLM
            logger.debug("Full context enabled; skipping filtering")
            filtered_journals = journals_data
            filtered_extras = extras_data
            filtered_emails = emails
            filtered_slack = slack_data
        elif budget_scope.lower() == "group":
            # apply the pipeline separately per source
            ...
            # rank all documents together to fill the global budget
            all_docs = journal_docs + extras_docs + email_docs + slack_docs
            # use a single ranking across all sources when applying a global budget
        ...
        if not full_context:
            # regroup filtered email and Slack docs back into the expected dict format
        ...
        # finally compose the prompt and call the LLM
