"""Generate a weekly narrative summary from journal and extras notes."""
...
def generate_weekly_summary(
    ...
    """Summarize a week of activity with token-budgeted compression."""
    ...
    try:
        ...
        def _score_entry(entry: dict) -> float:
            """Heuristically score an entry based on recency and information density."""
            ...
            for alias in alias_map.keys():
                # bump score if an alias is mentioned
            ...
            if "date" in entry and entry["date"]:
                # newer entries get a higher score
        ...
        def _compress_group(name: str, entries: List[dict], remaining: int) -> tuple[List[dict], int]:
            """Return summaries of ``entries`` while respecting ``remaining`` tokens."""
            if not entries or remaining <= 0:
                logger.warning("No budget left for %s", name)
                return [], 0

            # rank entries by score before summarizing
            ranked = sorted(entries, key=_score_entry, reverse=True)
            out: List[dict] = []
            used = 0
            for entry in ranked:
                # summarize long entries to save space
